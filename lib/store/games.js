const ActiveStore = require('./active');
...
const active = ({playerId, channelId, guildId}) =>
    Game.where({id: playerId, channelId, guildId, active: true})
        .then(result => result.length > 1 ? result : []);
...
const create = (message) => {
    ...
    return knex('games')
        ...
        .then(([id]) => {
            ...
            ActiveStore.set(guildId, channelId, player1.id, chess);
            ActiveStore.set(guildId, channelId, player2.id, chess);
            return Promise.all([
...
const load = (playerId, channelId, guildId) => {
    const [game] = active({playerId, channelId, guildId});
    if (game.pgn) {
        const chess = new Chess();
        chess.image = image;
        chess.load_pgn(game.pgn);
        chess.guild = {id: guildId, name: chess.header('DiscordGuild')};
        chess.channel = {id: channelId, name: chess.header('DiscordChannel')};
        chess.players = {
            black: game.black,
            white: game.white
        };
        ActiveStore.set(guildId, channelId, playerId, chess);
        return chess;
    }
    return undefined;
};
