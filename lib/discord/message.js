...
const move = async (message, author, input) => {
    ...
    if (chess.in_draw()) {
        gameOver(chess);
        return message.reply(`<@${chess.players.white}> has drew against <@${chess.players.black}>`);
    }

    if (chess.in_stalemate()) {
        gameOver(chess);
        return message.reply(`<@${chess.players.white}> has stalemated against <@${chess.players.black}>`);
    }

    if (chess.game_over()) {
        gameOver(chess);
        return message.reply(`Horray! <@${author}> has won!`);
    }
